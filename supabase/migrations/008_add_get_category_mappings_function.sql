...
LANGUAGE plpgsql
SECURITY INVOKER  -- Runs with caller's privileges (safer than SECURITY DEFINER)
...
BEGIN
  ...
  WHERE m.user_id = auth.uid()  -- RLS ensures users only see their own data
...
-- ============================================================================
-- BONUS: Auto-refresh schema cache on DDL changes (prevents future issues)
-- ============================================================================
...
LANGUAGE plpgsql
SECURITY DEFINER  -- OK here - just sends a notification, no data access
AS $$
BEGIN
